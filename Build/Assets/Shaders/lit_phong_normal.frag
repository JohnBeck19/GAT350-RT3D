...
#define OMISSIVE_TEXTURE_MASK	 (1 << 3) //1000
...
layout(binding = 3) uniform sampler2D omissiveTexture;
...
void phong(in Light light, in vec3 position, in vec3 normal, out vec3 albedo, out vec3 specular)
{
...
void main()
{
	...
	vec4 omissiveColor = bool(material.params & OMISSIVE_TEXTURE_MASK) ? texture(omissiveTexture, ftexcoord) : vec4(material.omissive, 1);
	//vec4 texcolor = texture(tex, ftexcoord);
	// set ambient light
	ocolor = vec4(ambientLight, 1) * albedoColor + omissiveColor;
